(get-info :version)
; (:version "4.8.12")
; Started: 2026-08-26 13:55:59
; Silicon.version: 1.1-SNAPSHOT (b40ab96f+@hollensteinj/path-sensitive_analysis)
; Input file: /workspaces/develop/precision_tests/viper_function_unused_precondition_by_parameter_type/fp_func_map_direct.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
(declare-fun unused_map_direct ($Snap Map<Int~_Int>) Bool)
(declare-fun unused_map_direct%limited ($Snap Map<Int~_Int>) Bool)
(declare-fun unused_map_direct%stateless (Map<Int~_Int>) Bool)
(declare-fun unused_map_direct%precondition ($Snap Map<Int~_Int>) Bool)
...
; Declaring symbols related to program functions (from verification)
(assert (! (forall ((s@$ $Snap) (m@0@00 Map<Int~_Int>)) (!
  (= (unused_map_direct%limited s@$ m@0@00) (unused_map_direct s@$ m@0@00))
  :pattern ((unused_map_direct s@$ m@0@00))
  :qid |quant-u-0|)) :named axiom_13))
(assert (! (forall ((s@$ $Snap) (m@0@00 Map<Int~_Int>)) (!
  (unused_map_direct%stateless m@0@00)
  :pattern ((unused_map_direct%limited s@$ m@0@00))
  :qid |quant-u-1|)) :named axiom_14))
(assert (! (forall ((s@$ $Snap) (m@0@00 Map<Int~_Int>)) (!
  (=>
    (unused_map_direct%precondition s@$ m@0@00)
    (= (unused_map_direct s@$ m@0@00) true))
  :pattern ((unused_map_direct s@$ m@0@00))
...
; m := Map(1 := 10)
; [eval] Map(1 := 10)
; [eval] Map[Int,Int]()[1 := 10]
; [eval] Map[Int,Int]()
(declare-const m@1@06 Map<Int~_Int>)
(assert (! (= m@1@06 (Map_update (as Map_empty  Map<Int~_Int>) 1 10)) :named assumption_37))
; [exec]
; assert unused_map_direct(m) == true
; [eval] unused_map_direct(m) == true
; [eval] unused_map_direct(m)
(push) ; 3
(declare-const $t@2@06 Map<Int~_Int>)
(assert (! (Map_equal $t@2@06 m@1@06) :named assumption_38))
; [eval] (1 in m)
(push) ; 4
(assert (! (not (Set_in 1 (Map_domain $t@2@06))) :named assertion_39))
(check-sat)
; unsat
(get-unsat-core)
; unsat core: (prover_33 prover_34 assumption_37 assumption_38 assertion_39)
(pop) ; 4
; 0.00s
; (get-info :all-statistics)
(assert (! (Set_in 1 (Map_domain $t@2@06)) :named assumption_40))
(assert (! (unused_map_direct%precondition $Snap.unit m@1@06) :named assumption_41))
(pop) ; 3
; Joined path conditions
(assert (! (and
  (Map_equal $t@2@06 m@1@06)
  (Set_in 1 (Map_domain $t@2@06))
  (unused_map_direct%precondition $Snap.unit m@1@06)) :named assumption_42))
(push) ; 3
(assert (! (not (= (unused_map_direct $Snap.unit m@1@06) true)) :named assertion_43))
(check-sat)
; unsat
(get-unsat-core)
; unsat core: (axiom_15 assumption_37 assumption_42 assertion_43)
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (! (= (unused_map_direct $Snap.unit m@1@06) true) :named assumption_44))
(pop) ; 2
(pop) ; 1
