...
(assert (! (= $t@2@06 $Snap.unit) :named assumption_37))
; [eval] x == 5
(assert (! (= x@1@06 5) :named assumption_38))
(pop) ; 2
(push) ; 2
; [exec]
; x := 5
(declare-const x@3@06 Int)
(assert (! (= x@3@06 5) :named assumption_39))
; [eval] x == 5
(push) ; 3
(assert (! (not (= x@3@06 5)) :named assertion_41))
(check-sat)
; unsat
(get-unsat-core)
; unsat core: (assertion_41 assumption_39)
(pop) ; 3
; 0.00s
