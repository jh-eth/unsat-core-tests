...
(check-sat)
; unknown
; ---------- FUNCTION set_inhale_body----------
(declare-fun s@0@00 () Set<Int>)
(declare-fun t@1@00 () Set<Int>)
(declare-fun u@2@00 () Set<Int>)
...
; [eval] (1 in s)
(assert (! (Set_in 1 s@0@00) :named assumption_4))
(assert (! (= ($Snap.second s@$) $Snap.unit) :named assumption_5))
; [eval] (2 in t)
(assert (! (Set_in 2 t@1@00) :named assumption_6))
(pop) ; 1
...
(assert (! (=> $$analysisLabel$$@6@00 (Set_in 1 s@0@00)) :named assumption_16))
(assert (! (=> $$analysisLabel$$@7@00 (= ($Snap.second s@$) $Snap.unit)) :named assumption_17))
(assert (! (=> $$analysisLabel$$@8@00 (Set_in 2 t@1@00)) :named assumption_18))
...
(assert (! (not (not (Set_in 1 s@0@00))) :named assertion_19))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
...
(push) ; 3
(set-option :rlimit 100000)
(assert (! (not (Set_in 1 s@0@00)) :named assertion_20))
(check-sat)
; unsat
(get-unsat-core)
...
; 0.00s
; (get-info :all-statistics)
; [then-branch: 0 | 1 in s@0@00 | live]
; [else-branch: 0 | !(1 in s@0@00) | dead]
(set-option :rlimit 0)
(push) ; 3
; [then-branch: 0 | 1 in s@0@00]
(assert (! (Set_in 1 s@0@00) :named assumption_21))
(pop) ; 3
(push) ; 3
; [else-branch: 0 | !(1 in s@0@00)]
(assert (! (not (Set_in 1 s@0@00)) :named assumption_22))
(set-option :rlimit 100000)
(check-sat)
; unsat
(get-unsat-core)
...
; Joined path conditions
(assert (! (or (not (Set_in 1 s@0@00)) (Set_in 1 s@0@00)) :named assumption_25))
(assert (! (= result@3@00 (ite (Set_in 1 s@0@00) 1 0)) :named assumption_26))
(pop) ; 1
(assert (! (forall ((s@$ $Snap) (s@0@00 Set<Int>) (t@1@00 Set<Int>) (u@2@00 Set<Int>)) (!
  (=>
